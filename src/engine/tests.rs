...
impl PaintObject<MockScreenPainter> for FakePaintObject {
    ...
    fn shift_with(&mut self, p: Vector2) {
        self.bounding_rect = self.bounding_rect.shifted_with(p);
...
fn nothing_is_drawn_if_no_tool_is_selected() {
    ...
    let mut tool1 = MockTool::new();
    tool1.expect_update().never().returning(|_, _, _| Ok(None));
    tool1.expect_draw().never().return_const(());
    engine.tools.push(Box::new(tool1));

    let mut tool2 = MockTool::new();
    tool2.expect_update().never().returning(|_, _, _| Ok(None));
...
fn only_the_selected_tool_is_used() {
    ...
    let mut tool1 = MockTool::new();
    tool1.expect_update().once().in_sequence(&mut seq).returning(|_, _, _| Ok(None));
    tool1.expect_draw().once().in_sequence(&mut seq).return_const(());
    engine.tools.push(Box::new(tool1));

    let mut tool2 = MockTool::new();
    tool2.expect_update().once().in_sequence(&mut seq).returning(|_, _, _| Ok(None));
...
fn no_selection_if_a_tool_is_selected() {
    let mut tool1 = MockTool::new();
    tool1.expect_update().returning(|_, _, _| Ok(None));
...
fn selection_disappears_after_selecting_a_tool() {
    ...
    tool.expect_update().returning(|_, _, _| Ok(None));
...
#[test]
fn moving_single_selected_object() {
    let tools = Vec::new();
    let view_width = 1000.0;
    let view_height = 1000.0;
    let mut engine = Engine::<MockScreenPainter, FakeIconType>::new(tools);
    ...
    engine.update(select_object1, STROKE, BG_COLOR, view_width, view_height);

    // control check: both objects are still where we put them
    assert_eq!(engine.objects[0].get_bounding_rect(), object1_rectangle);
...
#[test]
fn moving_multiple_selected_objects() {
    let tools = Vec::new();
    let view_width = 1000.0;
    let view_height = 1000.0;
    let mut engine = Engine::<MockScreenPainter, FakeIconType>::new(tools);
    ...
    assert!(engine.objects[0].is_selected());
    assert!(engine.objects[1].is_selected());
}
